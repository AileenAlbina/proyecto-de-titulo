...
@Component({
  selector: 'app-crear-al',
  templateUrl: './crear-al.component.html',
  styleUrls: ['./crear-al.component.css']
})
export class CrearAlComponent implements OnInit {

  
  public alumnoRegisterForm: FormGroup;
  ...
  public onSave(): void {
    ...
    this.apoderadoService.registrarApoderado(this.alumnoRegisterForm).subscribe(data => {
      if (data) {
  ...
  public isValidField(field: string): string {
    const validatedField = this.alumnoRegisterForm.get(field);
  ...
  private initDonorForm(): void {
    this.alumnoRegisterForm = this.fb.group({
